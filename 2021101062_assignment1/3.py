...
class Dataset:
    ...
    def add_extra_tokens(self):
        ...
        self.max_len = max([len(sentence) for sentence in self.words])
...
class Decoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, max_len, n_heads, ff_dim, n_layers=1, dropout=0.1):
        super(Decoder, self).__init__()
        self.input_dim = input_dim
        self.pos_encoder = PositionalEncoding(input_dim, max_len)
        self.decoder_layer = nn.TransformerDecoderLayer(d_model=input_dim, nhead=n_heads, dim_feedforward=ff_dim, dropout=dropout, batch_first=True)
        self.transformer = nn.TransformerDecoder(self.decoder_layer, num_layers=n_layers)
        self.fc = nn.Linear(input_dim, output_dim)
        
    def forward(self, x):
        mask = torch.full((x.size(1), x.size(1)), -float('inf'))
...
model = Decoder(input_dim, hidden_dim, output_dim, max_len, n_heads=4, ff_dim=512)
model.to(device)
...
perplexity(model, train_dataset, '2021101062-LM3-train-perplexity.txt')
perplexity(model, val_dataset, '2021101062-LM3-val-perplexity.txt')
perplexity(model, test_dataset, '2021101062-LM3-test-perplexity.txt')